...
from methods.common.creators.lseg.additional_utils.models import (
    resize_image,
    pad_image,
    crop_image,
)
...
def get_lseg_feat(
    ...
    if long_size <= crop_size:
        ...
        with torch.cuda.device_of(image):
            with torch.no_grad():
                outputs = (
                    image.new().resize_(batch, model.out_c, ph, pw).zero_().to(device)
                )
                logits_outputs = (
                    image.new().resize_(batch, len(labels), ph, pw).zero_().to(device)
                )
