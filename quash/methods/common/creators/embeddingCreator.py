...
import numpy as np
from methods.common.creators.networkFactory import NetworkFactory


class EmbeddingCreator:
    def __init__(
        self, device: str, network: str = "clip", model_name: str = "ViT-B/32"
    ):
        self.device = device
        if not device:
    ...
    def get_image_embedding(
        ...
        image_ready = (
            self.model.preprocess(Image.fromarray(image)).unsqueeze(0).to(self.device)
        )

        with torch.no_grad():
            embedding: torch.Tensor = self.model.encode_image(image_ready).to(
                self.device
            )
    ...
    def get_text_embedding(self, text: str) -> np.ndarray:
        ...
        with torch.no_grad():
            embedding: torch.Tensor = self.model.encode_text(text_tokens).to(
                self.device
            )
