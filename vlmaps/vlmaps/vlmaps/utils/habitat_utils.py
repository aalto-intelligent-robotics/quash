...
def make_cfg(settings: Dict) -> habitat_sim.Configuration:
    ...
    sim_cfg.scene_dataset_config_file = os.environ['VLMAPS_DIR'] + "/cfg/mp3d.scene_dataset_config.json"
...
def save_obs(
    root_save_dir: Union[str, Path], sim_setting: Dict, observations: Dict, save_id: int, obj2cls: Dict, obj2reg: Dict
) -> None:
    """
    ...
    # save semantic
    if sim_setting["regions"]:
        save_name = f"{save_id:06}.npy"
        save_dir = root_save_dir / "regions"
        os.makedirs(save_dir, exist_ok=True)
        save_path = save_dir / save_name
        obs = observations["semantic_sensor"]
        obs = cvt_obj_id_2_reg_id(obs, obj2reg)
        with open(save_path, "wb") as f:
            np.save(f, obs)
...
def get_obj2reg_dict(sim: habitat_sim.Simulator) -> Dict:
    """
    get the dictionary mapping from object id to semantic id
    """
    obj2cls = {}
    ...
    for obj in scene.objects:
        ...
        if(obj.region):
            parts = obj.region.id.split("_")
            level_id = parts[0]
            reg_id = parts[1]
            obj2cls[id] = (int(level_id), int(reg_id))
        else:
            obj2cls[id] = (int(-1), int(-1))
    return obj2cls
...
def cvt_obj_id_2_reg_id(semantic: np.ndarray, obj2cls: Dict) -> np.ndarray:
    h, w = semantic.shape
    semantic = semantic.flatten()
    u, inv = np.unique(semantic, return_inverse=True)
    return np.array([obj2cls[x][1] for x in u])[inv].reshape((h, w))
