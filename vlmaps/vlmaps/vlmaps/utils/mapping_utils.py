...
def save_3d_map(
    ...
    grid_histogram: Optional[Dict] = None,
) -> None:
    ...
    with h5py.File(save_path, "w") as f:
        ...
        if grid_histogram is not None:
            all_arrays = []
            id_list = []
            counts = []

            for k, v in grid_histogram.items():
                all_arrays.extend(v)
                id_list.append(k)
                counts.append(len(v))

            flat_data = np.stack(all_arrays)
            f.create_dataset("grid_hist_flat_data", data=flat_data)
            f.create_dataset("grid_hist_ids", data=np.array(id_list))
            f.create_dataset("grid_hist_counts", data=np.array(counts))
            # # works but is very slow
            # num_keys = 0
            # num_datasets = 0
            # for key, array_list in grid_histogram.items():
            #     num_keys += 1
            #     group_name = f"hist_{key}"
            #     group = f.create_group(group_name)
            #     for i, array in enumerate(array_list):
            #         group.create_dataset(str(i), data=array)
            #         num_datasets += 1
            # print(f"Created histogram with {num_keys} keys, {num_datasets} datasets")


def load_3d_map(
    map_path: str,
) -> Tuple[
    Set[int], np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, Dict
]:
    ...
    with h5py.File(map_path, "r") as f:
        ...
        if "grid_hist_flat_data" in f:
            grid_histogram = {}
            flat_data = f["grid_hist_flat_data"][()]
            ids = f["grid_hist_ids"][()]
            counts = f["grid_hist_counts"][()]

            idx = 0
            for key, count in zip(ids, counts):
                grid_histogram[int(key)] = [flat_data[i] for i in range(idx, idx + count)]
                idx += count
        # works but is very slow
        # num_keys = 0
        # num_datasets = 0
        ...
        #     if group_name.startswith("hist_"):
        #         key = int(group_name.split('_')[1])
        #         num_keys += 1
        #         group = f[group_name]
        #         arrays = [group[str(i)][()] for i in range(len(group))]
        #         grid_histogram[key] = arrays
        #         num_datasets += len(arrays)
        # print(f"Created histogram with {num_keys} keys, {num_datasets} datasets")
