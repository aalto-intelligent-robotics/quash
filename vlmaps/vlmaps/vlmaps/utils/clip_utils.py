...
import clip
from vlmaps.utils.cache_utils import load_cache, save_cache
...
def get_lseg_score(
    ...
    # debug caching
    # hash_object = [str(clip_model), landmarks, lseg_map, clip_feat_dim, use_multiple_templates, avg_mode, add_other]
    # found, obj = load_cache(hash_object)
    # if found:
    #     return obj
    zero_vector_key = "STT_ZERO_STT"

    if complement is None or not complement:
        landmarks_other = landmarks
        if add_other and landmarks_other[-1] != "other":
            landmarks_other = landmarks + ["other"]
    else:
        landmarks_other = landmarks
        if add_other and landmarks_other[-1] != complement[0]:
            landmarks_other = landmarks + complement

    if use_multiple_templates:
        zero_found = False
        idx = 0
        if zero_vector_key in landmarks_other:
            idx = landmarks_other.index(zero_vector_key)
            landmarks_other.remove(zero_vector_key)
            zero_found = True
        ...
        if zero_found:
            zero_vector = np.zeros((1, text_feats.shape[1]))
            text_feats_before = text_feats[:idx, :]
            text_feats_after = text_feats[idx:, :]
            text_feats = np.concatenate((text_feats_before, zero_vector, text_feats_after))
        ...
        # note: this is edited
        # scores_list = map_feats @ text_feats.T / (np.linalg.norm(map_feats, ord=2) * np.linalg.norm(text_feats, ord=2))
        # scores_list = np.zeros((map_feats.shape[0], text_feats.shape[0]))
        # for j in tqdm(range(text_feats.shape[0]), leave=False, desc="embed"):
        #     text_feat = text_feats[j, :]
        #     for i in range(map_feats.shape[0]):
        #         f = map_feats[i]
        #         div = np.linalg.norm(f, ord=2) * np.linalg.norm(text_feat, ord=2)
        #         if(div == 0):
        #             div = 1e-6
        #         cosine_similarity = np.dot(f, text_feat) / div
        #         scores_list[i, j] = cosine_similarity

    # save_cache(hash_object, scores_list)
    return scores_list
